...
export class ConfigReader {
  static loadConfig(): BenchmarkConfig {
    ...
    const config: BenchmarkConfig = {
      ...
      clientInstanceType: process.env.CLIENT_INSTANCE_TYPE || 'm5.xlarge',
      ...
      benchmarkCooldownDuration: process.env.BENCHMARK_COOLDOWN_DURATION || '10s',
  ...
  static validateConfig(config: BenchmarkConfig): void {
    ...
    // インスタンスタイプの検証
    const validInstanceTypes = /^[a-z]\d+[a-z]?\.\d*x?large$/;
    if (!validInstanceTypes.test(config.serverInstanceType)) {
      console.warn(`Warning: Unusual server instance type: ${config.serverInstanceType}`);
    }
    if (!validInstanceTypes.test(config.clientInstanceType)) {
      console.warn(`Warning: Unusual client instance type: ${config.clientInstanceType}`);
    }
